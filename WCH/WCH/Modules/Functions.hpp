...
extern string WCH_GetUserName();

typedef int(__stdcall* UDF)(LPVOID, LPCSTR, LPCSTR, DWORD, LPVOID);
UDF URLDownloadToFile = (UDF)GetProcAddress(LoadLibrary(L"urlmon.dll"), "URLDownloadToFileA");
...
void WCH_Init() {
    SetConsoleTitle(L"Web Class Helper");
...
void WCH_check_clock() {
    ...
    for (auto it = mm.equal_range(NOW.Hour).first; it != mm.equal_range(NOW.Hour).second; it++) {
        if ((it->second).first == NOW.Minute && ((it->second).second).size() > 0) {
            cout << "\a";
            MessageBoxA(NULL, ((it->second).second).c_str(), "Web Class Helper Clock", MB_OK);
        }
    }
}
...
void WCH_ow() {
    try {
        ...
        hFile = CreateFile(L"WCH_STDL.tmp", GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
